<?php
/*
  Plugin Name: WP Controller
  Plugin URI:
  Description: See and manage everything about your web business in one easy to use control panel.
  Author: campaigns.io
  Version: 2.8
  Author URI: http://campaigns.io
 */
...
if (version_compare(PHP_VERSION, '5.0.0', '<')) // min version 5 supported
    exit("<p>Wpcontroller Worker plugin requires PHP 5 or higher.</p>");
...
require_once("$mmb_plugin_dir/buddybackup.class.php");
//require_once("$mmb_plugin_dir/piwik.class.php");
...
if (!function_exists('mmb_response')) {
    ...
    function mmb_response($response = false, $success = true) {
        ...
        // print_r($return);
        // echo "</pre></code>";die;
        exit(base64_encode(serialize($return)));
    }
    ...
    function mmb_add_site($params) {
        ...
        if ($num) {
            if (!get_option('_action_message_id') && !get_option('_worker_public_key')) {
                $public_key = 'ccxvxwwewew34343cxvcxvdsfsdfds32432432432bcbccdsfds3432fgdfgasdsd';
                ...
                if (!get_option('_worker_nossl_key')) {
                    ...
                    mmb_response('Handshake not successful. Please deactivate, then activate Wpcontroller Worker plugin on your site, and re-add this site to your dashboard.', false);
            } else {
                mmb_response('Handshake not successful. Please deactivate, then activate Wpcontroller Worker plugin on your site, and re-add this site to your dashboard.', false);
            }
        } else {
...
if (!function_exists('mmb_remove_site')) {
    ...
    function mmb_remove_site($params) {
        ...
        if (!is_plugin_active($plugin_slug))
            mmb_response(array(
                'deactivated' => 'Site removed successfully. <br /><br />Wpcontroller Worker plugin successfully deactivated.'
                    ), true);
        else
            mmb_response(array(
                'removed_data' => 'Site removed successfully. <br /><br /><b>Wpcontroller Worker plugin was not deactivated.</b>'
                    ), true);
    }
...
if (!function_exists('mmb_clean_optimize_schedule')) {

    function mmb_clean_optimize_schedule($params) {
        global $mmb_core;
        $mmb_core->get_backup_instance();
        $return = $mmb_core->backup_instance->clean_optimize_schedule($params);
        if ($return)
            mmb_response($return, true);
        else
            mmb_response(false, false);
...
if (!function_exists('mwp_schedule_remote_backup')) {

    function mwp_schedule_remote_backup($params) {
        global $mmb_core;
        $mmb_core->get_backup_instance();
        $return = $mmb_core->backup_instance->schedule_remote_backup($params);
        if ($return)
            mmb_response($return, true);
        else
            mmb_response(false, false);
...
if (!function_exists('mwp_scheduled_optimize_tables')) {

    function mwp_scheduled_optimize_tables($params) {
        global $mmb_core;
        $mmb_core->get_backup_instance();
        $return = $mmb_core->backup_instance->optimize_tables_scheduled($params);
        if ($return)
            mmb_response($return, true);
        else
            mmb_response(false, false);
...
if (!function_exists('mmb_remote_backup_now')) {
    ...
    function mmb_remote_backup_now($params) {
        mmb_response(array('a' => '34234'), true);
        global $mmb_core;
...
/* wordfence */
if (!function_exists('mmb_wordfence_scan')) {

    function mmb_wordfence_scan($params) {
        global $mmb_core, $mmb_plugin_dir;
        require_once("$mmb_plugin_dir/wordfence.class.php");
        $mmb_core->get_wordfence_instance();
        $return = $mmb_core->wordfence_instance->scan($params);
        if (is_array($return) && array_key_exists('error', $return))
            mmb_response($return['error'], false);
        else {
            mmb_response($return, true);
...
if (!function_exists('mmb_wordfence_load')) {

    function mmb_wordfence_load($params) {
        global $mmb_core, $mmb_plugin_dir;
        require_once("$mmb_plugin_dir/wordfence.class.php");
        $mmb_core->get_wordfence_instance();
        $return = $mmb_core->wordfence_instance->load($params);
        if (is_array($return) && array_key_exists('error', $return))
            mmb_response($return['error'], false);
        else {
            mmb_response($return, true);
...
/* sucuri starts */
if (!function_exists('mmb_sucuri_scan')) {

    function mmb_sucuri_scan($params) {
        global $mmb_core, $mmb_plugin_dir;
        require_once("$mmb_plugin_dir/sucuri.class.php");
        $mmb_core->get_sucuri_instance();
        $return = $mmb_core->sucuri_instance->scan($params);
        if (is_array($return) && array_key_exists('error', $return))
            mmb_response($return, false);
        else {
            mmb_response($return, true);
...
if (!function_exists('mmb_upgrade_wp')) {

    function mmb_upgrade_wp($params) {
        global $mmb_core;
        $mmb_core->get_installer_instance();
        $return = $mmb_core->installer_instance->upgrade_wp($params);
        mmb_response($return, true);
    }
...
add_action('mwp_schedule_remote_backup', 'mwp_check_scheduled_remote_backup');
if (!function_exists('mwp_check_scheduled_remote_backup')) {

    function mwp_check_scheduled_remote_backup() {
        global $mmb_core, $_wp_using_ext_object_cache;
        $_wp_using_ext_object_cache = false;
        $mmb_core->get_backup_instance();
        $mmb_core->backup_instance->check_scheduled_remote_backup();
    }

}


add_action('mwp_scheduled_optimize_tables', 'mwp_check_scheduled_optimize_tables');
if (!function_exists('mwp_check_scheduled_optimize_tables')) {

    function mwp_check_scheduled_optimize_tables() {
        global $mmb_core, $_wp_using_ext_object_cache;
        $_wp_using_ext_object_cache = false;
        $mmb_core->get_backup_instance();

        $mmb_core->backup_instance->check_scheduled_optimize_tables();
    }
...
if (!function_exists('mmb_get_autoupdate_plugins_themes')) {
    ...
    function mmb_get_autoupdate_plugins_themes($params) {
        $return = Controller::getSettings($params);
        mmb_response($return, true);
    }
...
if (!function_exists('mmb_edit_autoupdate_plugins_themes')) {
    ...
    function mmb_edit_autoupdate_plugins_themes($params) {
        $return = Controller::setSettings($params);
        mmb_response($return, true);
    }
...
if (!function_exists('mmb_buddy_backup_schedule')) {

    function mmb_buddy_backup_schedule($params) {

        global $mmb_core;
        $mmb_core->get_backupbuddy_instance();
        $return = $mmb_core->backupbuddy_instance->addSchedule($params);
        mmb_response($return, true);
    }

}

if (!function_exists('mmb_buddy_backup_deleteschedule')) {

    function mmb_buddy_backup_deleteschedule($params) {
        global $mmb_core;
        $mmb_core->get_backupbuddy_instance();
        $return = $mmb_core->backupbuddy_instance->deleteSchedule($params);
        mmb_response($return, true);
    }
...
// if (!function_exists('mmb_add_piwik_code')) {

//     function mmb_add_piwik_code($params) {
//         global $mmb_core;
//         // adding the piwik option
//         $mmb_core->get_piwik_instance();
//         $return = $mmb_core->piwik_instance->addpiwikcode($params);
//         mmb_response($return, true);
//     }
...
if (!function_exists('mmb_buddy_backup_delete_accounts')) {

    function mmb_buddy_backup_delete_accounts($params) {
        global $mmb_core;

        $mmb_core->get_backupbuddy_instance();
        $return = $mmb_core->backupbuddy_instance->deleteDestination($params);
        mmb_response($return, true);
    }
...
if (!function_exists('mmb_itheme_security')) {
    ...
    function mmb_itheme_security($params) {
        ...
        if (is_plugin_active('ithemes-security-pro/ithemes-security-pro.php')) {
            ...
            $protect_files = $itsec_tweaks['protect_files'];
            $suspicious_query_strings = $itsec_tweaks['suspicious_query_strings'];
            $generator_tag = $itsec_tweaks['generator_tag'];
            $comment_spam = $itsec_tweaks['comment_spam'];
            $disable_xmlrpc = $itsec_tweaks['disable_xmlrpc'];
            $uploads_php = $itsec_tweaks['uploads_php'];
            $disable_unused_author_pages = $itsec_tweaks['disable_unused_author_pages'];
            $directory_browsing = $itsec_tweaks['directory_browsing'];
            $request_methods = $itsec_tweaks['request_methods'];
            $non_english_characters = $itsec_tweaks['non_english_characters'];
            $long_url_strings = $itsec_tweaks['long_url_strings'];
            $write_permissions = $itsec_tweaks['write_permissions'];
            $wlwmanifest_header = $itsec_tweaks['wlwmanifest_header'];
            $edituri_header = $itsec_tweaks['edituri_header'];
            $random_version = $itsec_tweaks['random_version'];
            $file_editor = $itsec_tweaks['file_editor'];
            $login_errors = $itsec_tweaks['login_errors'];

            //Medium priority
            $four_of_four=get_option('itsec_four_oh_four');
            $itsec_away_mode=get_option('itsec_away_mode');
            $itsec_file_change=get_option('itsec_file_change');
            $itsec_hide_backend=get_option('itsec_hide_backend');
            $itsec_online_files=get_option('itsec_online_files');
            $table_repfix=$wpdb->prefix;
            if($table_repfix!='wp_')
                $fixed_table_prefix=1;
            else
                $fixed_table_prefix='';

            //Low priority
            $itsec_ban_users=get_option('itsec_ban_users');
            $itsec_ssl=get_option('itsec_ssl');
            $itsec_user_logging=get_option('itsec_user_logging');
            ...
            //Medium Priority
            $return['medium_priority']['four_oh_four'] = $four_of_four['enabled'];
            $return['medium_priority']['itsec_away_mode'] = $itsec_away_mode['enabled'];
            $return['medium_priority']['itsec_file_change'] = $itsec_file_change['enabled'];
            $return['medium_priority']['itsec_hide_backend'] = $itsec_hide_backend['enabled'];
            $return['medium_priority']['protect_files'] = $protect_files;
            $return['medium_priority']['suspicious_query_strings'] = $suspicious_query_strings;
            $return['medium_priority']['generator_tag'] = $generator_tag;
            $return['medium_priority']['comment_spam'] = $comment_spam;
            $return['medium_priority']['disable_xmlrpc'] = $disable_xmlrpc;
            $return['medium_priority']['uploads_php'] = $uploads_php;
            $return['medium_priority']['disable_unused_author_pages'] = $disable_unused_author_pages;
            $return['medium_priority']['table_prefix'] = $fixed_table_prefix;
            $return['medium_priority']['itsec_online_files'] = $itsec_online_files['enabled'];
            // Low proirity
            $return['low_priority']['itsec_ban_users'] = $itsec_ban_users['enabled'];
            $return['low_priority']['itsec_ssl'] = $itsec_ssl['login'];
            $return['low_priority']['directory_browsing'] = $directory_browsing;
            $return['low_priority']['request_methods'] = $request_methods;
            $return['low_priority']['non_english_characters'] = $non_english_characters;
            $return['low_priority']['long_url_strings'] = $long_url_strings;
            $return['low_priority']['write_permissions'] = $write_permissions;
            $return['low_priority']['wlwmanifest_header'] = $wlwmanifest_header;
            $return['low_priority']['edituri_header'] = $edituri_header;
            $return['low_priority']['random_version'] = $random_version;
            $return['low_priority']['file_editor'] = $file_editor;
            $return['low_priority']['login_errors'] = $login_errors;
            $return['low_priority']['itsec_user_logging'] = $itsec_user_logging['enabled'];
...
if(!function_exists('mmb_get_broken_links')) {
    function mmb_get_broken_links() {
        global $wpdb;
        $prefix = $wpdb->prefix;
        $links = $wpdb->get_results(
            "SELECT final_url,http_code,link_text,post_title,guid
                FROM
                    {$prefix}blc_links link,
                    {$prefix}blc_instances i,
                    {$prefix}posts post
                WHERE link.broken= 1
                    AND i.link_id = link.link_id
                    AND i.container_id = post.ID
            ",ARRAY_A
        );
        $broken_links = array();
        if($links){
            ...
            {
                $array = array(
                    'broken_link' => $link['final_url'],
                    'status' => $link['http_code'],
                    'link_text' => $link['link_text'],
                    "linked_post_title" => $link['post_title'],
                    'linked_post_url' => $link['guid']
                );
                array_push($broken_links, $array);
            }
        } else {
            $broken_links = 'no-data';
        }
        mmb_response($broken_links, true);
    }
}
